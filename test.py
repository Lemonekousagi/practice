...
#モデルクラスの定義
class CNN(nn.Module): #Netの部分は任意だが、必ずnn.Moduleを継承
  def __init__(self, n_output, n_hidden):
    super().__init__()
    self.conv1 = nn.Conv2d(3, 32, 3) #入力チャネル数(最初はRGBなので3)、出力チャネル数、フィルタサイズ
    self.conv2 = nn.Conv2d(32, 64, 3)
    self.maxpool = nn.MaxPool2d((2, 2)) #プーリング層
    self.flatten = nn.Flatten() #多次元配列を1次元に変換

    self.fc1 = nn.Linear(2304, n_hidden) #全結合層
    self.fc2 = nn.Linear(n_hidden, n_output)

    self.relu = nn.ReLU(inplace = True)
  
  def forward(self, x):
    x1 = self.maxpool(self.relu(self.conv1(x))) #畳み込み層1
    x2 = self.maxpool(self.relu(self.conv2(x1))) #畳み込み層2
    x3 = self.flatten(x2) #多次元配列を1次元に変換
    x4 = self.relu(self.fc1(x3)) #全結合層1
    x5 = self.fc2(x4) #全結合層2
    return x5
